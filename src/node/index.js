...
/**
 * Build a safe HTTP response body for frontend clients.
 * By default, stack traces are only included outside production.
 *
 * @param {unknown} error
 * @param {{
 ...
 * }} [options]
 */
export function buildClientErrorResponse(error, options = {}) {
  const env = options.environment || process.env.NODE_ENV || "development";
  const includeStack =
    typeof options.includeStack === "boolean"
      ? options.includeStack
      : env !== "production";
  const safeMessage = options.defaultMessage || "internal server error";
  ...
  if (includeStack && error && typeof error === "object" && "stack" in error) {
